...
for (upper, lower) in gameRepository.getUppercasedAndLowercasedNames() {
...
let highestScoreSum = gameRepository.getAll().map({ $0.highestScore }).reduce(0, {
    sum, highestScore in
    return sum + highestScore
})
print(highestScoreSum)

let highestScoreSum2 = gameRepository.getAll().map({ $0.highestScore }).reduce(0, {
    sum, highestScore in
...
let highestScoreSum3 = gameRepository.getAll().map({ $0.highestScore }).reduce(0) {
    sum, highestScore in
    print("sum value: \(sum), highestScore value: \(highestScore)")
    return sum + highestScore
}

let highestScoreSumFor500000 = gameRepository.summedHighestScoresWhere(minPlayedCount: 500_000)
